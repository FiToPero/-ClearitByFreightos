...
const newMessage = (id) => { router.get(route('message.user', [id]), {}, {}) }
const sabeTicket = (id, status) => { console.log(status), router.post(route('ticket.update.user', [id, status]), {}, {})  }
const sabeMessage = (id) => { router.post(route('message.update.user', [id]), {}, {}) }
                                        ...
                                        <tbody>
                                            <tr v-for="message in ticket.messages" :key="message.id">
                                                ...
                                                <td class="border px-4 py-2">{{ message.is_read ? 'Yes' : 'No' }}
                                                    <PrimaryButton 
                                                        v-if="message.type && message.type.includes('agent') && (message.is_read === null || message.is_read === undefined)" 
                                                        class="m-2 p-2" 
